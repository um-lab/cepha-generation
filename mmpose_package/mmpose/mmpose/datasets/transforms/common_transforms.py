...
@TRANSFORMS.register_module()
class GenerateRegTarget(BaseTransform):
    def __init__(self, heatmap_size, ref_keypoints):
        super().__init__()
    ...
    def transform(self, results: Dict) -> Optional[Union[Dict, Tuple[List, List]]]:
        ...
        # target_kpts = [key - 1 for key in self.ref_keypoints]
        # reference_kpts = [x - 1 for x in self.ref_keypoints]
        # ind = []
        # transformed_keypoints = []

        ind_all = []  # 用于regression的参考点在offsetmap上的位置
        offsets_all = []
        transformed_keypoints = []
        kpts = results['transformed_keypoints'].copy()

        if results.get('transformed_keypoints', None) is not None:
            transformed_keypoints = results['transformed_keypoints'].copy()
            scale_factor = np.array(results['input_size']) / self.heatmap_size
            transformed_keypoints = np.round(np.round(transformed_keypoints) / scale_factor)

        for key in ref_keypoints_index:
            ind = []
            for i in ref_keypoints_index[key]:
                ct = transformed_keypoints[0][i]
                ct_int = ct.astype(np.int32)
                ind.append(ct_int[1] * w + ct_int[0])
            ind_all.append(ind)
            offsets = kpts[0, key, :] - kpts[0, ref_keypoints_index[key], :]
            offsets[:, 0] /= results['input_size'][0]
            offsets[:, 1] /= results['input_size'][1]
            offsets_all.append(offsets)

        ind_all = np.array(ind_all)
        results['ind'] = ind_all.reshape(1, *ind_all.shape)
        offsets_all = np.array(offsets_all)
        results['offsets'] = offsets_all.reshape(1, *offsets_all.shape)
        # torch.save([results, transformed_keypoints], '/home/jianbingshen/guodongqian/CL-Detection2023-MMPose/debug_reg/GenerateRegTarget.pth')
