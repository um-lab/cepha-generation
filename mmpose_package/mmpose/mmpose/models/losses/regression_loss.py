...
def _transpose_and_gather_feat(feat, ind):
    feat = feat.permute(0, 2, 3, 1).contiguous()  # 交换维度次序
    feat = feat.view(feat.size(0), -1, feat.size(3))  # 将宽和高合并为一个维度
...
class RegLoss(nn.Module):
    ...
    def forward(self, output, ind, target=None):
        pred = _transpose_and_gather_feat(output, ind)
        loss = self._reg_loss(pred, target)
        return loss

    def _reg_loss(self, regr, gt_regr):
        """ L1 regression loss
          Arguments:
            regr (batch x max_objects x dim)
            gt_regr (batch x max_objects x dim)
        """
        num = regr.size(1) * regr.size(0)
